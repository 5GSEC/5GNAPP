...
@tool
def search_mitre_fight_techniques(threat_summary: str, top_k: int=5, fight_json_path: str="mitre_fight_techniques.json", embedding_model_name="all-MiniLM-L6-v2") -> list:
    '''
...
def load_or_create_mitre_fight_faiss_index(fight_json_path: str="mitre_fight_techniques.json", fight_db_name: str="mitre_fight.faiss_index", embedding_model_name="all-MiniLM-L6-v2"):
    """
    Load or create a FAISS index for MITRE FiGHT techniques.
    """
    if os.path.exists(fight_db_name):
        print("Loading existing FAISS index from file...")
        index = faiss.read_index("mitre_fight.faiss_index")
        return index
