...
def fetch_service_status_osc() -> dict:
    ''' 
        Fetch the status of the SE-RAN services, including xApps and others, the at OSC near-RT RIC.
...
@tool
def fetch_service_status_osc_tool() -> dict:
    ''' 
...
@tool
def fetch_sdl_event_data_osc_tool() -> dict:
    ''' Fetch network event data generated by MobieXpert and MobiWatch from SDL
        Returns:
            dict: A dictionary containing the network event data
    '''
...
@tool
def build_xapp_osc_tool(xapp_name: str):
    """
    Build the xApp from the given xapp_name.
    Steps:
        step 1: get xapp_name
